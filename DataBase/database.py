...
class Database:
    ...
    def init_schema(self):
        with self.conn.cursor() as cur:
            # Если нормализованной схемы ещё нет (старая БД или первый запуск) —
            # сносим прежние таблицы и создаём заново. После этого данные
            # переживают перезапуски (повторного DROP не будет).
            cur.execute("SELECT to_regclass('public.categories')")
            has_new = cur.fetchone()[0] is not None
            if not has_new:
                cur.execute("""
